...
import com.tcc.unisolidaria.databinding.ActivityProfileBinding
            ...
            clientProvider.uploadImage(authProvider.getId(), imageFile!!).addOnSuccessListener { taskSnapshot ->
                clientProvider.getImageUrl().addOnSuccessListener { url ->
                    ...
                    clientProvider.update(client).addOnCompleteListener {
                        if (it.isSuccessful) {
                            Toast.makeText(this@ProfileActivity, "Dados atualizados com sucesso", Toast.LENGTH_LONG).show()
                        }
                        else {
                            Toast.makeText(this@ProfileActivity, "Não foi possível atualizar as informações", Toast.LENGTH_LONG).show()
                        }
                    }
